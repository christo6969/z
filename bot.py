...
class ClasseVivaMonitor:
    ...
    Uses EXACT login() and get_communications() methods with FIXED webidentity extraction
    """
    ...
    def login(self) -> bool:
        ...
        CRITICAL FIX: Extract webidentity from JSON response as per problem statement:
        - Extract from: data['data']['auth']['accountInfo']['type'] + data['data']['auth']['accountInfo']['id'] + 'W'
        - Example: if type='S' and id='10269016', webidentity='S10269016W'
        """
        try:
            ...
            if data and not data.get('error'):
                # CRITICAL FIX: Extract webidentity from JSON response (lines 327-333 from local_monitor.py)
                # Example: if type='S' and id='10269016', webidentity='S10269016W'
                try:
                    account_type = data['data']['auth']['accountInfo']['type']
                    account_id = data['data']['auth']['accountInfo']['id']
                    self.webidentity = f"{account_type}{account_id}W"
                    log_colored(f"LOGIN: webidentity estratto da JSON: {self.webidentity}", Fore.CYAN)
                except (KeyError, TypeError) as e:
                    log_colored(f"ERRORE: Impossibile estrarre webidentity da JSON - {str(e)}", Fore.RED)
                    return False
                
                # CRITICAL FIX: Extract PHPSESSID from session cookies (lines 335-342 from local_monitor.py)
                # Iterate through self.session.cookies and extract PHPSESSID
                for cookie in self.session.cookies:
                    if cookie.name == 'PHPSESSID':
                        self.phpsessid = cookie.value
                        log_colored(f"LOGIN: PHPSESSID estratto dai cookie: {self.phpsessid[:10]}...", Fore.CYAN)
                        break
                
                if not self.phpsessid:
                    log_colored("ERRORE: PHPSESSID non trovato nei cookie", Fore.RED)
                    return False
